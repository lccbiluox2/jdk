...
class WindowsUserPrincipals {
    ...
    static class User implements UserPrincipal {
         ...
         * SID的字符串形式
         *
         * 注：SID(security identifier)用来标识user、user group和account信息
         */
        private final String sidString; // String representation of SID

        // SID类型
        private final int sidType; // SID type

        // account名称；如果account不可用，则存储SID
        private final String accountName; // Account name (if available) or SID
        ...
        // 返回account名称；如果account不可用，则返回SID
        @Override
    ...
    // 文件"用户组"，这里只是简单地继承了User，因为"用户组"信息与"所有者"信息可以使用同一套API
