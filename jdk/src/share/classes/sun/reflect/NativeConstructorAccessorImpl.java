 ...
 */
class NativeConstructorAccessorImpl extends jdk.internal.reflect.ConstructorAccessorImpl {
    private final Constructor<?> constructor;           // 待访问构造器
    private jdk.internal.reflect.DelegatingConstructorAccessorImpl parent;   // 当前类的代理
    ...
    public Object newInstance(Object[] args) throws InstantiationException, IllegalArgumentException, InvocationTargetException {
        ...
        if(++numInvocations>threshold && !ReflectUtil.isVMAnonymousClass(constructor.getDeclaringClass())) {
            jdk.internal.reflect.MethodAccessorGenerator accessor = new jdk.internal.reflect.MethodAccessorGenerator();
            jdk.internal.reflect.ConstructorAccessorImpl acc = (jdk.internal.reflect.ConstructorAccessorImpl) accessor.generateConstructor(constructor.getDeclaringClass(), constructor.getParameterTypes(), constructor.getExceptionTypes(), constructor.getModifiers());
    ...
    void setParent(jdk.internal.reflect.DelegatingConstructorAccessorImpl parent) {
        this.parent = parent;
    }
